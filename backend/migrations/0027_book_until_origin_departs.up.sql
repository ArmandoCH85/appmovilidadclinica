-- ============================================================================
-- 0027_book_until_origin_departs.up.sql
-- Regla A: se puede buscar y reservar un viaje MIENTRAS EL BUS NO HAYA SALIDO
-- de la parada de origen pedida — aunque el viaje ya esté en curso.
--
-- Motivación: hoy el pasajero no encontraba el viaje de las 18:35 (Surco->Lima)
-- a las 18:35 aunque el bus ni había salido de Surco (parada ARRIVED). Dos
-- bloqueos:
--   1) sp_search_trips exigía trip.status = 'PUBLISHED' -> el viaje desaparece
--      del buscador apenas arranca (IN_PROGRESS).
--   2) sp_confirm_reservation exigía la ventana de reserva abierta
--      (booking_closes_at = 30 min antes de la salida) -> rechazaba igual.
--
-- Cambios:
--   sp_search_trips:
--     * status IN ('PUBLISHED','BOARDING','IN_PROGRESS')
--     * la parada de origen no debe haber salido: actual_departure_at IS NULL
--       y no estar SKIPPED.
--     * booking_state: OPEN mientras el origen no haya salido (ya no depende
--       de booking_closes_at; el límite pasa a ser la salida real del origen).
--   sp_confirm_reservation:
--     * acepta status IN ('PUBLISHED','BOARDING','IN_PROGRESS')
--     * mantiene el chequeo de apertura (booking_opens_at)
--     * reemplaza el corte por booking_closes_at por el chequeo "el origen ya
--       salió / fue omitido".
--
-- El inventario es por segmento, así que vender un tramo posterior en un viaje
-- en curso no pisa asientos ya usados en tramos previos.
-- ============================================================================
...
BEGIN
    SELECT trip.id AS trip_id,
           ...
           -- 0027: el límite real es la salida del origen, no booking_closes_at.
           CASE
               WHEN origin_stop.actual_departure_at IS NOT NULL
                    OR origin_stop.status = 'SKIPPED' THEN 'CLOSED'
     ...
     WHERE trip.service_date = p_service_date
       ...
       -- 0027: antes era solo 'PUBLISHED'.
       AND trip.status IN ('PUBLISHED', 'BOARDING', 'IN_PROGRESS')
       -- 0027: el bus todavía no salió de la parada de origen pedida.
       AND origin_stop.actual_departure_at IS NULL
       AND origin_stop.status <> 'SKIPPED'
...
BEGIN
    ...
    DECLARE v_origin_departed DATETIME;
    DECLARE v_origin_status VARCHAR(20);
    ...
    -- 0027: antes solo 'PUBLISHED'. Ahora también viajes ya iniciados.
    IF v_trip_status NOT IN ('PUBLISHED', 'BOARDING', 'IN_PROGRESS') THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'El viaje no admite reservas';
    END IF;

    -- 0027: se mantiene la apertura; el corte por booking_closes_at se
    -- reemplaza por "el bus todavía no salió de tu parada de origen".
    IF CURRENT_TIMESTAMP < v_booking_opens_at THEN
        SIGNAL SQLSTATE '45000'
    ...
    SELECT origin_time.stop_order,
           ...
           origin_time.actual_departure_at,
           origin_time.status,
    ...
    -- 0027: el bus ya pasó (o se omitió) la parada de origen.
    IF v_origin_departed IS NOT NULL OR v_origin_status = 'SKIPPED' THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'El bus ya pasó por tu parada de origen';
    END IF;
    ...
    -- Reglas direccionales (ver 0026): IDA -> destino SEDE; VUELTA -> sin
    -- restricción de tipo de origen (lo que manda es pickup_allowed).
