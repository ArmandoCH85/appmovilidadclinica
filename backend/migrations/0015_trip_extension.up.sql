-- ============================================================================
-- 0015_trip_extension.up.sql
-- Extension de viaje del pasajero: evento EXTENDED, columna de destino
-- original y vista de disponibilidad enriquecida.
--
-- Idempotente: se puede re-ejecutar sin romper. En deploys limpios 0001 ya
-- crea la columna y la vista; aca se repiten con guardas.
-- Requiere MariaDB (ADD COLUMN IF NOT EXISTS). El server es MariaDB 11.4.5.
-- ============================================================================

-- 1. Amplia el ENUM de eventos para registrar extensiones.
...
-- 2. Columna que guarda el destino original antes de la primera extension.
ALTER TABLE reservations
    ADD COLUMN IF NOT EXISTS original_destination_stop_order SMALLINT UNSIGNED NULL;

-- 3. Vista de disponibilidad enriquecida (DROP + CREATE es idempotente).
