-- ============================================================================
-- 0028_remove_mark_date_guard.up.sql
-- QUITA el filtro de fecha de las marcas de parada (llegada y salida).
--
-- Motivo: estamos en fase de PRUEBAS y el app del conductor permite navegar
-- otros dias y iniciar esos viajes. El guard 0024 bloqueaba la marca con
-- "La marca de llegada esta fuera del dia de servicio del viaje" cuando el
-- service_date del viaje no era hoy (p.ej. viaje del 01/10 operado el 21/09).
--
-- Que se conserva:
--   * solo el conductor asignado puede marcar,
--   * el viaje debe estar iniciado (BOARDING/IN_PROGRESS),
--   * COMPLETED/CANCELLED no admiten marcas,
--   * (salida) primero debe registrarse la llegada.
-- Que se quita:
--   * la validacion "DATE(marca) BETWEEN service_date AND service_date+1".
--
-- TEMPORAL: re-agregar el guard cuando termine la fase de pruebas (basta con
-- aplicar el .down de esta migracion, que restaura la definicion de 0024).
-- ============================================================================
