-- ============================================================================
-- 0026_allow_paradero_origin_vuelta.up.sql
-- Alinea la VALIDACIÓN DE RESERVA con la BÚSQUEDA.
--
-- Contexto: la migración 0011 permitió subir en paraderos de las rutas VUELTA
-- (relajó sp_search_trips a "(IDA y destino SEDE) OR (VUELTA)"), pero dejó
-- sp_confirm_reservation con la regla estricta original:
--
--     IF v_direction = 'VUELTA' AND v_origin_type <> 'SEDE' THEN SIGNAL ...
--
-- Resultado: el pasajero ENCONTRABA el viaje pero NO podía reservarlo
-- ("En VUELTA, el origen de la reserva debe ser una SEDE").
--
-- Este cambio elimina esa validación para VUELTA. El resto queda igual:
--   - IDA sigue exigiendo destino SEDE.
--   - El tramo debe ser válido (origen < destino).
--   - pickup_allowed / dropoff_allowed siguen mandando sobre dónde se sube y baja.
-- ============================================================================
