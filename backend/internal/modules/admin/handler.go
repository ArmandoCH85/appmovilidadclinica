...
// (IDA/VUELTA), stop_id y vehicle_id (placa/bus).
func (h *AdminHandler) BoardingsByStopReport(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.svc.GetBoardingsByStop(r.Context(), q.Get("date_from"), q.Get("date_to"), routeID, q.Get("direction"), stopID, vehicleID)
	if err != nil {
		apperror.WriteJSONError(w, err)
		return
	}
